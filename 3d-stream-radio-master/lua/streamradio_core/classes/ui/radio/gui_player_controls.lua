...
-- huge tracks dont need playbar updates every frame
...
function CLASS:Create()
	...
	-- seek sync is still sketchy here. server has no BASS, so this lies a bit
	self.PlayBar.OnFractionChangeEdit = function(this, v)
		...
		-- tiny noise forces the net update
...
function CLASS:SetStream(stream)
	...
	if CLIENT then
		local function UpdatePlayBar()
			...
			-- kick one redraw right after state change
			self:SetFastThinkRate(0)
...
function CLASS:UpdateFromStream()
	...
	-- yeah this part is a bit cursed. server cant verify seek state cleanly without BASS.
...
function CLASS:UpdatePlaybackLoopMode(loopMode)
	...
	if not StreamOBJ:HasEnded() then
		...
		-- keep time stable when loop mode flips
