...
function ENT:GetCursor( ply, trace, userEntity )
	...
	-- wire can proxy input through non-player entities
...
function ENT:PollGuiSetup()
	...
	-- stagger gui rebuilds, big spawn piles spike hard
...
function ENT:SyncPlaylistState()
	...
	self:SetHasPlaylist(hasPlaylist)

	if not IsValid(self.GUI_Main) then
		return
	end

	self.GUI_Main:SetHasPlaylist(hasPlaylist)
end
...
function ENT:PlayFromPlaylistItem(playlistItem)
	...
	if self._nextPlaylistSwitch and self._nextPlaylistSwitch > now then
		-- dont let next/prev spam melt this
		return
	end
...
if CLIENT then
	...
	function ENT:ReapplyPlaylistFromDupe()
		-- weird dupe timing: gui init can stomp playlist, so reapply once later
		...
		StreamRadioLib.Timer.Once(timerId, 1, function()
			...
			-- drop it late so init races dont wipe it
			self._dupePlaylistData = nil
		end)
